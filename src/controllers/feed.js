                        ...
                        database.query("SELECT * FROM tb_ideia", (err3, rows3, fields3) => {
                            if(err3){
                                ...
                                if(ideias_pesquisadas.length == []){
                                    ...
                                    let sql3 = "SELECT m.id_mensagem, m.ct_mensagem, m.id_ideia, u.id_usuario, u.nm_usuario, DATE_ADD(m.hr_mensagem, INTERVAL - 2 HOUR) hr_mensagem FROM tb_mensagem m JOIN tb_usuario u on u.id_usuario = m.id_usuario WHERE uso_mensagem = 2 AND ("
