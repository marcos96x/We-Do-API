                ...
                if (err2) {
                    ...
                    database.query("SELECT * FROM membros_ideias WHERE id_ideia = ?", [id_ideia], (err3, rows3, fields3) => {
                        if (err3) {
                            ...
                            database.query("SELECT m.id_mensagem, m.ct_mensagem, m.id_ideia, u.nm_usuario, TIME(DATE_ADD(m.hr_mensagem, INTERVAL - 0 HOUR)) hr_mensagem FROM tb_mensagem m JOIN tb_usuario u on u.id_usuario = m.id_usuario WHERE uso_mensagem = 2 AND id_ideia = ?", id_ideia, (err4, rows4, fields4) => {
                                if (err4) {
